...
class rocket_control(object):
    ...
    def slow_descend_controll(self, dt=0.05):
        ...
        # target_point0_ = (- self.velocity_surf / (self.speed_surf + 1e-5)).copy()
        # if self.surface_altitude > 30:
    ...
    def rocket_height_maintainence(self, 
        ...
        # 最小允许俯仰角正切值
        min_tan_theta_allowed = np.sqrt(np.abs(self.max_twr**2 - 1))
    ...
    def single_rocket_thread(self, r):
        ...
        while self.running:
            ...
            try:
                ...
                # 根据模式执行对应的控制逻辑
                if mode == "HEIGHT":
                    r.rocket_height_maintainence(target_height=h, target_N_speed=vn, target_E_speed=ve, dt=self.dt, type='simple')
                elif mode == "LANDING":
