...
class rocket_control(object):
    ...
    def get_state_of_NUE(self,):
        ...
        # 当前推重比
        self.current_twr = self.vessel.thrust / (self.vessel.mass * self.vessel.orbit.body.surface_gravity)
        # 最大推重比
        self.max_twr = self.vessel.available_thrust / (self.vessel.mass * self.vessel.orbit.body.surface_gravity)
        # 真空比冲
...
if __name__ == '__main__':
    ...
    for i in range(int(40/dt)):
        ...
        if t < 10:
            ...
            for i, Rocket in enumerate(Rocket_list):
                Rocket.slow_descend_controll(dt=dt)
        time.sleep(dt)