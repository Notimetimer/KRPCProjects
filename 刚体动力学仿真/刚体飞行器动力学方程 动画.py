...
class FlyingObject:
    ...
    def move2(self, F_b_, M_b_, dt):
        ...
        # 还是把力搬到惯性系去算质心运动
        F_i = left_multiply(self.R_i2b.T, F_b_)  # 先用最新的姿态把力转到惯性系
        v_i_dot_ = F_i / self.m     # 在惯性系下，加速度纯粹就是 F/m，没有向心力补偿项
        ...
        # v_i_next_ = self.v_ + (v_i_dot_ + self.v_dot_last_)/2 * dt # （旋转惯性系下不要用）梯形积分1
        # self.v_dot_last_ = v_i_dot_
        if norm(self.v_) > 1e-3:
            v_i_dot_mag = np.dot(v_i_dot_, self.v_)/(norm(self.v_)+1e-8)
            v_i_mag = norm(self.v_) + v_i_dot_mag * dt
            v_i_next_ = v_i_next_/(norm(v_i_next_) + 1e-8) * v_i_mag
...
if __name__ == '__main__':
    ...
    p0, v0 = np.array([0,0,0]), np.array([200,0,0])
