...
class MultiArms:
    ...
    def reset(self):
        ...
        self.phis = np.zeros(len(self.lengths0))
        self.thetas = np.zeros(len(self.lengths0))
        self.psis = np.zeros(len(self.lengths0))
    ...
    def set_state(self,
        ...
        # 设置特定状态并更新运动学
        self.lengths = self.lengths0 + deltaX
        self.phis = phis.copy()
    ...
    def forward_kinematics(self):
        ...
        for i in range(len(self.lengths0)):
            # 每个关节相对于上一级的旋转
            Rotation_i = RotMat_zyx(self.phis[i], self.thetas[i], self.psis[i])
            # 累积旋转矩阵 (从基座到当前关节)
            Rotation_matrix = Rotation_i @ prev_Rotation_matrix
            
            # 计算当前杆件在全局坐标系下的末端位置
            curr_x = np.array([self.lengths[i], 0, 0])
            current_joint_ = prev_joint_ + left_multiply(Rotation_matrix.T, curr_x)
    ...
    def move_wt_f(self, dt, 
        ...
        for i in range(len(self.lengths0)):
            ...
            # 根据角速度前进
            self.phis[i] += phi_dots[i] * dt
            self.thetas[i] += theta_dots[i] * dt
            self.psis[i] += psi_dots[i] * dt
...
if __name__=='__main__':
    ...
    deltaX0 = np.array([0, 0, 0, 0])
    
    # # 设置一些旋转角度来测试 (弧度)
    phis0 = np.radians([0, 30, 30, 30])
    thetas0 = np.radians([0, 0, 0, 0])
    psis0 = np.radians([0, 90, 0, 90])
    ...
    def update(frame):
        ...
        # 初始化角度数组
        phi_dots = np.zeros(4)
        theta_dots = np.zeros(4)
        psi_dots = np.zeros(4)
        
        # 运动逻辑设定:
        # 1. 臂1 和 臂2：在 psi 方向做缓慢的正弦运动
        phi_dots[0] += (np.radians(120) * sin(0.5 * t) - arms1.phis[0]) * 100 * dt
        psi_dots[0] += (np.radians(20) * sin(0.5 * t) - arms1.psis[0]) * 100 * dt
        psi_dots[1] += (np.radians(60) + np.radians(20) * sin(0.5 * t) - arms1.psis[1]) * 100 * dt
        
        # 2. 臂3：在 theta 方向做正弦运动
        theta_dots[2] += (np.radians(60) * cos(0.5 * t) - arms1.thetas[2]) * 100 * dt
        psi_dots[2] += (np.radians(60) * cos(0.5 * t) - arms1.psis[2]) * 100 * dt

        # 3. 臂4：psi 锁定为 90°
        phi_dots[3] = 5.0
        psi_dots[3] += (np.radians(90) - arms1.psis[3])  * 100  * dt
        
        
        
        # 使用 set_state 更新状态，内部会自动调用 forward_kinematics 并重置关节列表
        length_dots = np.zeros(4)
        arms1.move_wt_f(dt, length_dots, phi_dots, theta_dots, psi_dots)
    ...
    # 创建动画
    # 创建动画，显式设置 cache_frame_data=False 以避免警告
